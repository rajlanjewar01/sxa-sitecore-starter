//TODO
/**
 * custom hooks(window object not found in dom - custom hooks to find the window breakpoints)
 * useMemo, useRef, context
 */
...
const AppleNavbar = ({ fields }: NavbarProps): JSX.Element => {
	...
	const handleMouseEnter = (item: NavItemData) => {
	...
	return (
	  ...
	  >
		<nav className={styles.navbar}>
		  ...
		  <ul className={`${styles.navbar__list} ${isMobileMenuOpen ? styles.navbar__list_visible : ''}`}>
			{navItems.map((item) => (
			  <MenuItem 
				key={item.id} 
				item={item} 
